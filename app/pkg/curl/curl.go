...
func NewHTTPClient() IHttpClient {
	transCfg := &http.Transport{
		TLSHandshakeTimeout: 1 * time.Minute, //change tls timeout handshake into 1 minute
	}
	return &http.Client{Transport: transCfg}
}

func NewInsecureHTTPClient() IHttpClient {
...
func (rs *HttpResponse) IsSuccess() bool {
	if rs.response.StatusCode == 200 || rs.response.StatusCode == 201 || rs.response.StatusCode == 202 || rs.response.StatusCode == 204 {
		return true
	}
